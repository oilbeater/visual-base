...
indexes, future tool-use over the video — comes *out* of it, never
replaces it. If it was on your screen, it's in the recording.
...
flowchart LR
    ...
    V -->|watcher detects<br/>finalization| K["Kimi video<br/>understanding"]
    K -->|per-segment .md<br/>merged by date| D[("Daily log<br/>YYYY-MM-DD.md")]
```

The `.mp4` segments are the source of truth — the daily `.md` log
is derived from them, and can always be regenerated by re-running
the understanding step on the same segments.
